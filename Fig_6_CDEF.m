%% Code for Figs 6C-F: effects of surveillance on the age-dependent risk profile and in combination with contact reducing NPIs
% Francesca Lovell-Read (francesca.lovell-read@merton.ox.ac.uk)
% Version of: Monday 26th April 2021

% This code computes the age-dependent local outbreak probabilities given
% specified rates of surveillance for symptomatic and/or nonsymptomatic hosts.
% It plots a bar graph of these probabilities, overlaid on bars corresponding
% to the baseline case. The weighted average outbreak probability P is also
% plotted. This code also computes local outbreak probabilities under specified
% surveillance measures in combination with reductions in 'work' and 'other'
% contacts ranging between 0% and 100%. These probabilities are plotted as a
% heat map, with contours along which the outbreak probability is constant
% overlaid.
clear
fprintf('\n\t***RUNNING***\n');
...
% Baseline isolation rate of symptomatic hosts ('rho'): one entry per age group
rho_bl = zeros(1,16);
% Baseline isolation rate of nonsymptomatic hosts ('sigma'): one entry per age group
sigma_bl = zeros(1,16);

% DEFINE CONTROLS:
% Specify string describing control implemented (to be used in figure legend)
% control_implemented = 'Symptomatic surveillance';
control_implemented = 'Surveillance for all hosts';
% Isolation rate of symptomatic hosts ('rho')
rho_controls = zeros(1,16)+1/2;
% Isolation rate of nonsymptomatic hosts ('sigma')
sigma_controls = zeros(1,16)+1/7;

% Vectors for varying strength of 'work' and 'other' controls (0 = no reduction, 1 = 100% reduction)
work_strength_vec = 0:0.1:1;
other_strength_vec = 0:0.1:1;
...
A_cont = sum(N.*J.*xi./(nu+sigma_bl).*R_row_sums);
P_cont = sum(N.*J.*(1-xi)./(lambda+sigma_bl).*R_row_sums);
S_cont = sum(N.*J.*(1-xi)./(lambda+sigma_bl).*lambda./(mu+rho_bl).*R_row_sums);

% Compute transmission rate scaling factors eta, theta
eta = (S_cont/P_cont)*Kp/(1-Ka-Kp);
theta = (S_cont/A_cont)*Ka/(1-Ka-Kp);

multiplier_2 = ((theta*xi)./(nu+sigma_bl))+((1-xi)./(lambda+sigma_bl)).*(eta+lambda./(mu+rho_bl));
multiplier_3 = sum(multiplier.*R_row_sums.*multiplier_2);
B = R0/multiplier_3;
S = (B)*(J.*R_row_sums);

%% COMPUTE BASELINE OUTBREAK PROBABILITIES --------------------------------
...
%% COMPUTE OUTBREAK PROBABILITIES WITH SPECIFIED SURVEILLANCE -------------
fprintf('\tComputing outbreak probability with specified surveillance...\n');
rho = rho_controls;
sigma = sigma_controls;
...
p_presymp = p_all(17:32);
p_asymp = p_all(33:48);
p_2 = xi.*p_asymp+(1-xi).*p_presymp;
% Compute weighted average PLO
PLO_control = sum(p_2.*N_prop);

%% PLOT BAR GRAPH ---------------------------------------------------------
fprintf('\tPlotting...\n');
figure(); hold on; 

myplot(1) = bar(p,'Facecolor',[0.9 0.9 0.9],'Edgecolor',[0.5 0.5 0.5],'Linewidth',1);
myplot(2) = bar(p_2,'Facecolor',[0.7 0 1],'FaceAlpha',0.2,'Edgecolor',[0 0 0],'Linewidth',1);
myplot(3) = plot([0 17],[PLO_control PLO_control],'color',[0.5 0 0.8],'linewidth',3);
myplot(4) = plot([0 17],[PLO PLO],'color',[0 0 0],'linewidth',3,'linestyle','-.');

Legend=cell(2,1);
Legend{1}=strcat('All contacts');
Legend{2}=strcat(control_implemented);
leg = legend(myplot([4,3]),Legend,'Fontsize',20,'interpreter','tex');
leg.Title.String = 'Average outbreak probability (P)';
leg.Box = 'on';

xlabel('Age group of index case','Fontsize',16); xlim([0.5 16.5]);
xticklabels = ({'0'; '10'; '20'; '30'; '40'; '50'; '60'; '70'; '80'});
xticks = linspace(0.5, 16.5, numel(xticklabels));
set(gca, 'XTick', xticks, 'XTickLabel', xticklabels);

ylabel('Probability of local outbreak (p_k)','Fontsize',16); ylim([0 1]);
yticklabels = ({'0'; '0.2'; '0.4'; '0.6'; '0.8'; '1'});
yticks = linspace(0,1, numel(yticklabels));
set(gca, 'YTick', yticks, 'YTickLabel', yticklabels);

box off; set(gca,'Fontsize',20,'Linewidth',2);
fprintf('\n\t***DONE!***\n\n');

%% VARYING WORK AND OTHER CONTROLS WITH SPECIFIED SURVEILLANCE ------------
fprintf('\tComputing outbreak probability with varied controls...\n');
M_control = zeros(length(other_strength_vec),length(work_strength_vec));

for i=1:length(other_strength_vec)
    o_s = other_strength_vec(i);
    for j=1:length(work_strength_vec)
        w_s = work_strength_vec(j);

        C_controls = C-w_s*C_work-o_s*C_other;
        rho = rho_controls;
        sigma = sigma_controls;
        ...
        p_presymp = p_all(17:32);
        p_asymp = p_all(33:48);
        p_3 = xi.*p_asymp+(1-xi).*p_presymp;

        % Compute weighted average PLO
        M_control(i,j) = sum(p_3.*N_prop);

    end
end

%% PLOT HEAT MAP WITH VARIED CONTROLS -------------------------------------
fprintf('\tPlotting...\n');
figure(); 

% Contour plot of local outbreak probabilities
levels = [0 0.01 0.05 0.1 0.15 0.2 0.25 0.3 0.35 0.4 0.45];
...
colbar.Ticks = [0 0.1 0.2 0.3];
caxis([0 0.38]);

xlabel('Reduction in other contacts (%)');
xticklabels = ({'0'; '20'; '40'; '60'; '80'; '100'});
xticks = linspace(1, length(other_strength_vec), numel(xticklabels));
