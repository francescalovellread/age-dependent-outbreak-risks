%% Code for Figs 6A,B: effects of symptomatic and nonsymptomatic surveillance
% Francesca Lovell-Read (francesca.lovell-read@merton.ox.ac.uk)
% Version of: Monday 26th April 2021

% This code varies the isolation rate of symptomatic hosts and nonsymptomatic
% hosts between 0 days^-1 and 1 days^-1, first individually and then 
% simultaneously, computing the local outbreak probability at each point. The
% outbreak probabilities obtained from varying the symptomatic and nonsymptomatic
% isolation rates individually are plotted on a single graph for comparison.
% The probabilities obtained from varying both simultaneously are plotted as a
% heat map, with contours along which the outbreak probability is constant
% overlaid. Contains data for each of the three scenarios A,B,C.
clear
fprintf('\n\t***RUNNING***\n');
...
% Vectors for varying surveillance rates
rho_control_vec = 0:0.05:1; %Symptomatic surveillance
sigma_control_vec = 0:0.05:1; % Nonsymptomatic surveillance
...
%% VARYING ISOLATION RATE OF SYMPTOMATIC HOSTS ----------------------------
fprintf('\tVarying symptomatic surveillance...\n');
PLO_rho_control = zeros(1,length(rho_control_vec));

for i=1:length(rho_control_vec)
    
    C = C_controls; 
    rho = zeros(1,16)+rho_control_vec(i);
    sigma = zeros(1,16);
    R = bsxfun(@times,C,K); % Scales each column in the contact matrix by the relevant susceptibility
    R_row_sums = sum(R,2)';  % Computes the row sums of the scaled matrix R
    multiplier = J.*N_prop;
    ...
    % Compute weighted average PLO
    PLO_rho_control(i) = sum(p.*N_prop);
end

%% VARYING ISOLATION RATE OF NONSYMPTOMATIC HOSTS -------------------------
fprintf('\tVarying nonsymptomatic surveillance...\n');
PLO_sigma_control = zeros(1,length(sigma_control_vec));

for i=1:length(rho_control_vec)

    C = C_controls; 
    rho = zeros(1,16);
    sigma = zeros(1,16)+sigma_control_vec(i);
    ...
    % Compute weighted average PLO
    PLO_sigma_control(i) = sum(p.*N_prop);
end

%% PLOT: ONE WAY VARIATION ------------------------------------------------
fprintf('\tPlotting...\n');
figure(); hold on;

myplot(1) = plot(rho_control_vec,PLO_rho_control,'r','linewidth',3);
myplot(2) = plot(sigma_control_vec,PLO_sigma_control,'b','linewidth',3); 

xlabel('Isolation rate (days^{-1})');
ylabel('Average outbreak probability (P)');

leg = legend([myplot(1) myplot(2)],'Symptomatic hosts','Nonsymptomatic hosts');
leg.Box = 'on'; leg.Location = 'northeast'; leg.FontSize = 20;

box off; set(gca,'Fontsize',20,'Linewidth',2);
fprintf('\n\t***DONE!***\n\n');

%% VARYING ISOLATION RATE OF BOTH SIMULTANEOUSLY --------------------------
fprintf('\tVarying surveillance...\n');
M_control = zeros(length(rho_control_vec),length(sigma_control_vec));

for i=1:length(rho_control_vec);
    rho = zeros(1,16)+rho_control_vec(i);
    for j=1:length(sigma_control_vec)
        sigma = zeros(1,16)+sigma_control_vec(j);
...
xlabel('Symptomatic isolation rate (days^{-1})');
ylabel('Nonsymptomatic isolation rate (days^{-1})');

xticklabels = ({'0'; '0.2'; '0.4'; '0.6'; '0.8'; '1'});
xticks = linspace(1, length(rho_control_vec), numel(xticklabels));
set(gca, 'XTick', xticks, 'XTickLabel', xticklabels, 'Fontsize', 16);

yticklabels = ({'0'; '0.2'; '0.4'; '0.6'; '0.8'; '1'});
yticks = linspace(1, length(rho_control_vec), numel(xticklabels));
