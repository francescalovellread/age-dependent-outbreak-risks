%% Code for Fig S14: the effects non-independent contacts
% Francesca Lovell-Read (francesca.lovell-read@merton.ox.ac.uk)
% Version of: Wednesday 13th October 2021

% This code simultaneously varies the increase in 'home' contacts and the decrease
% in 'work' contacts that may occur as a result of total school closures between
% 0% and 100%, for a specified reduction in 'other' contacts. It plots a heat map
% of the outbreak probabilities, overlaid with contours along which the outbreak
...
% Reduction in 'other' contacts
% o_s = 0.2; % A: 20% reduction in 'other' contacts
o_s = 0; % B: no change in 'other' contacts
% o_s = -0.2; % C: 20% increase in 'other' contacts

% Vectors for varying strength of 'school' and 'work' controls (0 = no reduction, 1 = 100% reduction)
home_strength_vec = 0:0.2:1;
...
M_control = zeros(length(home_strength_vec),length(work_strength_vec)); % For storing values

for i=1:length(home_strength_vec)
    h_s = home_strength_vec(i);
    for j=1:length(work_strength_vec)
        w_s = work_strength_vec(j);

        % Compute contact matrix under specified controls
        C_controls = C-C_school-w_s*C_work+h_s*C_home-o_s*C_other;
...
levels = [0 0.05 0.1 0.15 0.2 0.25 0.3 0.35 0.4 0.45 0.5 0.55]; % Specify contours to plot
...
set(colbar,'linewidth',2,'fontsize',20,'XTick', [0.2 0.3 0.4 0.5]);
caxis([0.2 0.5]); % Set colorbar limits

xlabel('Increase in home contacts (%)');
xticklabels = ({'0'; '20'; '40'; '60'; '80'; '100'});
xticks = linspace(1, length(home_strength_vec), numel(xticklabels));
