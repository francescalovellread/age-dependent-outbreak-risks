%% Code for Fig 5D: varying reduction in 'other' contacts
% Francesca Lovell-Read (francesca.lovell-read@merton.ox.ac.uk)
% Version of: Monday 26th April 2021

% This code varies the reduction in 'other' contacts between 0% and 100% and
% calculates the corresponding weighted average local outbreak probability P
% in the following two situations: i) when there is no reduction in 'school'
% and 'work' contacts, and ii) when there is a 100% reduction in 'school' and
% 'work' contacts. It plots the probabilities for each situation on the same
% graph for comparison. Contains data for each of the three scenarios A,B,C.
clear
fprintf('\n\t***RUNNING***\n');
...
% Vector for varying strength of 'other' controls (0 = no reduction, 1 = 100% reduction)
other_strength_vec = 0:0.02:1;
...
%% CALCULATE LOCAL OUTBREAK PROBABILITIES ---------------------------------
fprintf('\tComputing local outbreak probabilities...\n');

% All school and work contacts
other_control = zeros(1,length(other_strength_vec));

for i = 1:length(other_strength_vec)
    o_s = other_strength_vec(i);

    C_controls = C-o_s*C_other; 
...
other_control_2 = zeros(1,length(other_strength_vec));

for i = 1:length(other_strength_vec)
    o_s = other_strength_vec(i);

    C_controls = C-C_school-C_work-o_s*C_other; 
    R = bsxfun(@times,C_controls,K); % Scales each column in the contact matrix by the relevant susceptibility
    R_row_sums = sum(R,2)';  % Computes the row sums of the scaled matrix R
    multiplier = J.*N_prop;
    S = (B)*(J.*R_row_sums);
    ...
    % Compute weighted average PMO
    other_control_2(i) = sum(p.*N_prop);
end

%% PLOT -------------------------------------------------------------------
fprintf('\tPlotting...\n');
figure(); hold on;
myplot(1) = plot(other_strength_vec,other_control,'color',[0 0.8 0 1],'linestyle',':','linewidth',3);
myplot(2) = plot(other_strength_vec,other_control_2,'color',[0 0.8 0 1],'linewidth',3);
...
yticks = linspace(0, 0.5, numel(yticklabels));
set(gca, 'YTick', yticks, 'YTickLabel', yticklabels, 'Fontsize', 16);

leg = legend([myplot(1) myplot(2)],'All school and work contacts','No school or work contacts','location','northeast');
