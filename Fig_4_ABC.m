%% Code for Figs 4A-C: effects of contact-reducing NPIs on age-dependent outbreak probabilities
% Francesca Lovell-Read (francesca.lovell-read@merton.ox.ac.uk)
% Version of: Monday 26th April 2021

% This code uses the UK 'all contacts' matrix and its breakdown into 
% 'school', 'work', 'home' and 'other' contacts to demonstrate the effects
% of removing different types of contact on the local outbreak probability.
% It uses the specified population R0 along with the 'all contacts' matrix
% to calculate the scaling factor 'B'. It then uses this same 'B' to compute
% new outbreak probabilities corresponding to the specified controlled contact
% matrix. It plots a bar graph of these controlled probabilities, overlaid
% on the original 'all contacts' probabilities for comparison. It also plots
% the weighted average P for both the controlled case and the original case.
% Contains data for each of the three scenarios A,B,C.
clear
fprintf('\n\t***RUNNING***\n');
...
control_implemented = 'No "school" contacts';
% Define modified contact matrix taking desired controls into account
C_controls = C-C_school;
% Isolation rate of symptomatic hosts ('rho')
rho_controls = zeros(1,16);
% Isolation rate of nonsymptomatic hosts ('sigma')
sigma_controls = zeros(1,16);

% Bar and line colours for plotting (comment in/out as appropriate)
barColor = [1 0 0]; lineColor = [0.8 0 0 1]; % Colours for 'school'
% barColor = [0 0 1]; lineColor = [0 0 0.8 1]; % Colors for 'work'
% barColor = [0 1 0]; lineColor = [0 0.8 0 1]; % Colours for 'other'
...
%% RECALCULATE USING SAME B BUT NEW CONTACT MATRIX, WITH CONTROLS ---------
fprintf('\tComputing outbreak probability with controls...\n');
C = C_controls; 
rho = rho_controls;
...
% Compute vector of PLOs
p_all_2 = 1-r;
p_symp_2 = p_all_2(1:16);
p_presymp_2 = p_all_2(17:32);
p_asymp_2 = p_all_2(33:48);
p_2 = xi.*p_asymp_2+(1-xi).*p_presymp_2;
...
myplot(2) = bar(p_2,'Facecolor',barColor,'FaceAlpha',0.2,'Edgecolor',[0 0 0],'Linewidth',1);
myplot(3) = plot([0 17],[PLO_control PLO_control],'color',lineColor,'linewidth',3);
...
percentage_diff = 100*(PLO-PLO_control)/PLO;
R_new = 1/(1-PLO_control);

% Print output values:
fprintf(strcat('\tPLO (no controls):','\t',num2str(PLO),'\n'));
fprintf(strcat('\tPLO (controls):','\t\t',num2str(PLO_control),'\n'));
fprintf(strcat('\tPercentage diff:','\t',num2str(percentage_diff),'\n'));
fprintf(strcat('\tR_0 (controls):','\t\t',num2str(R_new),'\n'));
